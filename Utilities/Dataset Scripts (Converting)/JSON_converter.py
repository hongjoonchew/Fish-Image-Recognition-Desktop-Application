...
json_list = ['1-100','dataset3_76to150','151-200'] #list of all the annotations we currently have
...
for json_name in json_list:
	...
	for i in range(len(d)):
		output_dir = os.getcwd() + '/augmented_dataset/train/'
		training_count += 1
		if random.random() < val_ratio:
			output_dir = os.getcwd() + '/augmented_dataset/val/'
			training_count -= 1
			val_count+= 1
		fn = str(d[i]['filename'])
		shutil.copy(os.getcwd() + '/' + json_name +'/' + fn,output_dir+ 'images/') #Copy from source to dst
